...
public class TurnosService : ITurnosService
{
    ...
    {
        return _mapper.Map<TurnoReadDto>(await GetAsync(id));
    }

    public async Task<IEnumerable<TurnoReadDto>> GetDtoByPrestadorAsync(Guid prestadorId)
    {
        var turnos = await _context.Turnos.Where(t => t.PrestadorId == prestadorId).OrderBy(t => t.FechaHoraInicioUtc).ToListAsync();

        return _mapper.Map<IEnumerable<TurnoReadDto>>(turnos);
    }

    public async Task<DisponibilidadFechaDto> GetDisponibilidadDtoAsync(Guid? prestadorId, Guid sedeId, DateTime fecha, Guid servicioId)
    {
    ...
    public async Task<bool> CambiarEstadoAsync(Guid turnoId, EstadoTurnoEnum nuevoEstado)
    {
        ...
        if (exito && nuevoEstado == EstadoTurnoEnum.Cancelado)
        {
            ...
            if (!string.IsNullOrEmpty(turno.RecordatorioJobId))
            {
                ...
                {
                    _logger.LogWarning(ex, "Falló la cancelación del recordatorio de Hangfire para el turno {TurnoId}", turno.Id);
                }
            }
    ...
    public async Task<bool> CancelarPorClienteAsync(Guid turnoId)
    {
        ...
        int horasMinimas = 1; //minimo de cancelacion 1 hora antes. parametrizar
    ...
    public async Task<DashboardResumenDto> GetDashboardResumenAsync()
    {
        ...
        var turnos = await _context.Turnos
            .AsNoTracking()
            .Where(t => t.NegocioId == negocioId && t.FechaHoraInicioUtc >= inicioMesAnteriorUtc && t.FechaHoraInicioUtc < inicioMesSiguienteUtc && t.Estado != EstadoTurnoEnum.Cancelado)
            .Select(t => new
            {
        ...
        var ingresosCaja = await _context.MovimientosCaja
            .AsNoTracking()
            .Where(m => m.NegocioId == negocioId && m.CreatedAtUtc >= inicioMesAnteriorUtc && m.CreatedAtUtc < inicioMesSiguienteUtc && m.Tipo == TipoMovimientoEnum.Ingreso)
            .Select(m => new
            {
                m.CreatedAtUtc,
                m.Monto
    ...
    public async Task<byte[]> GenerarReporteExcelAsync(DateTime desde, DateTime hasta)
    {
        ...
        var turnos = await _context.Turnos
            .AsNoTracking() // uso de AsNoTracking para reportes de solo lectura
            .Include(t => t.Cliente)
            .Include(t => t.Prestador)
            .Include(t => t.Servicio)
            .Where(t => t.NegocioId == negocioId && t.FechaHoraInicioUtc >= desdeUtc && t.FechaHoraInicioUtc <= hastaUtc)
            .OrderBy(t => t.FechaHoraInicioUtc)
            .ToListAsync();
    ...
    public async Task<TurnoReadDto> CrearTurnoAdminAsync(TurnoAdminCreateDto dto)
    {
        ...
        if (turnoOcupado) throw new InvalidOperationException("Este horario choca con otra reserva existente.");
        ...
        if (chocaConBloqueo) throw new InvalidOperationException("Este horario pisa un bloqueo de agenda (vacaciones/ausencia).");
    ...
    private void DispararSincronizacionGoogle(string? refreshToken, string? calendarId, string nombreCliente, string? telefonoCliente, string? emailCliente, string nombreServicio, DateTime inicioUtc, DateTime finUtc, Guid prestadorId, bool esReservaManualAdmin)
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                await _googleCalendarService.CrearEventoAsync(refreshToken, calendarId ?? "primary", tituloEvento, descripcionEvento, inicioUtc, finUtc);
            }
            catch (Exception ex)
            {
    ...
    internal async Task<Turno> GetAsync(Guid id)
    {
    ...
    internal async Task<Turno?> FindAsync(Guid id)
    {
